...
class Pool:
    ...
    async def request(self, workstation, request, *, now_ms):
        self.calls.append((workstation, dict(request), now_ms))
        return {
            "ok": True,
...
async def test_gateway_is_thin_projection_over_existing_pool():
    ...
    assert [call[2] for call in pool.calls] == [1234, 1234]
...
async def test_process_exec_requires_explicit_gateway_operation_grant():
    ...
    assert pool.calls[0][1]["operation"] == "process.exec"
    assert pool.calls[0][1]["request_id"] == "process-1"
