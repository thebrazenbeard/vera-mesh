...
class MirroredReadLaneRouter:
    ...
    async def renew(self, *, lane_id: str, fencing_token: int, ttl_s: float = 300.0) -> dict[str, Any]:
        lane = self._require(lane_id, fencing_token)
        if ttl_s <= 0:
            raise ValueError("ttl_s must be positive")
        lane.expires_at_ms = self.now_ms() + int(ttl_s * 1000)
        endpoints = {item.config.endpoint_id: item for item in self._ordered_current_endpoints()}
        for endpoint_id, (session_id, mirror_fence) in tuple(lane.mirrors.items()):
            ...
            if endpoint is None or endpoint.binding.session_id != session_id:
                ...
                continue
            try:
                response = await endpoint.channel.request({
                    "protocol_version": "veraport-v1",
    ...
    async def close(self, *, lane_id: str, fencing_token: int) -> dict[str, Any]:
        ...
        for endpoint_id, (session_id, mirror_fence) in tuple(lane.mirrors.items()):
            ...
            if endpoint is None or endpoint.binding.session_id != session_id:
                continue
            try:
                await endpoint.channel.request({
                    "protocol_version": "veraport-v1",
        ...
        return {
            ...
            "request_id": None,
            "ok": True,
            "result": {"lane_id": lane_id, "closed": True, "logical_read_lane": True},
        }
