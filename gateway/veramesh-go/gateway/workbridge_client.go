...
func (c WorkBridgeUpstreamConfig) Validate() error {
	...
	if len(c.AllowedRoots) == 0 {
		return errors.New("WorkBridge allowed_roots must be non-empty")
	}
	if _, err := normalizeWorkBridgeRoots(c.AllowedRoots); err != nil {
		return err
	}
	return nil
}
...
func NewWorkBridgeClient(ctx context.Context, cfg WorkBridgeUpstreamConfig) (*WorkBridgeClient, error) {
	...
	allowedRoots, err := normalizeWorkBridgeRoots(cfg.AllowedRoots)
	if err != nil {
		_ = session.Close()
		return nil, err
	}
...
func normalizeWorkBridgeRoots(values []string) ([]string, error) {
	...
	for _, raw := range values {
		canon, err := CanonicalRemotePath(raw)
		if err != nil {
			return nil, fmt.Errorf("invalid WorkBridge allowed root %q: %w", raw, err)
		}
		key := canon
		if workBridgeCaseInsensitivePath(canon) {
	...
	if len(out) == 0 {
		return nil, errors.New("WorkBridge allowed_roots must contain at least one valid root")
	}
	return out, nil
}

func workBridgeCaseInsensitivePath(value string) bool {
	return (len(value) >= 3 && value[1] == ':' && value[2] == '/') || strings.HasPrefix(value, "//")
}

func workBridgePathWithinRoot(candidate, root string) bool {
	if workBridgeCaseInsensitivePath(candidate) || workBridgeCaseInsensitivePath(root) {
...
func (w *WorkBridgeClient) pathAllowed(raw string) bool {
	if w == nil || len(w.allowedRoots) == 0 {
		return false
	}
	canon, err := CanonicalRemotePath(raw)
	if err != nil {
	...
	for _, root := range w.allowedRoots {
		if workBridgePathWithinRoot(canon, root) {
			return true
		}
	}
	return false
