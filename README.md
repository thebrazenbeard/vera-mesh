...
- Interactive workstation access is direct-first, persistent-session-first, mutually authenticated, and multiplexed.
...
The hot-session candidate adds:
- P-256 mutual proof of possession between a controller and exact workstation;
- both-side nonce binding and replay resistance;
- local workstation capability-ceiling enforcement;
- explicit protocol-version/key-shape rejection;
- one persistent framed stream carrying multiple concurrent requests with request/result correlation.
...
- `protocol/veraport/v1/hot-session-profile.md`
